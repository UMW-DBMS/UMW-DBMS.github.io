...
// Point types contained in the "Water features-point" source layer. Each
// checkbox filters the same GeoJSON by its hydrographic feature code.
const waterFeaturePointTypes = [
    { name: 'Anicut', code: 'ANCTP' }, { name: 'Flood Gate', code: 'FLGAP' },
    { name: 'Sluice', code: 'SLCEP' }, { name: 'Spill', code: 'SPLLP' },
    { name: 'Waterfall', code: 'WTRFP' }, { name: 'Culvert', code: 'CLVTP' },
    { name: 'Spring', code: 'SPRNP' }, { name: 'Water Tower', code: 'TOWRP' },
...
const waterFeatureLineTypes = [
    ...
    { name: 'Tunels', codes: ['TNNLL'], color: '#4292c6' },
    { name: 'Others', codes: [], color: '#6baed6' }
];
const waterFeatureLineDataCache = new Map();

function isWaterFeaturesPointLayer(layerName) {
    return String(layerName || '').trim().toLowerCase().replace(/[\s-]+/g, '') === 'waterfeaturespoint';
}

function isWaterFeaturesPolygonLayer(layerName) {
    return String(layerName || '').trim().toLowerCase().replace(/[\s-]+/g, '') === 'waterfeaturespolygon';
}
...
function getWaterFeatureLineCode(properties) {
    return String((properties || {}).HL_GFCODE || '').trim().toUpperCase();
}

function loadWaterFeatureLineType(url, layerName, lineType) {
    const cacheKey = String(url || '').trim();
    const dataPromise = waterFeatureLineDataCache.has(cacheKey) ? waterFeatureLineDataCache.get(cacheKey) : fetch(url).then((response) => {
        if (!response.ok) throw new Error('Network response was not ok');
        return response.json();
    });
    waterFeatureLineDataCache.set(cacheKey, dataPromise);

    dataPromise.then((data) => {
        const knownCodes = waterFeatureLineTypes.flatMap((type) => type.codes);
        const filteredData = {
            ...data,
            features: (data.features || []).filter((feature) => {
                const code = getWaterFeatureLineCode(feature.properties);
                return lineType.name === 'Others' ? !knownCodes.includes(code) : lineType.codes.includes(code);
            })
        };
        const layer = L.geoJSON(filteredData, {
            style: { color: lineType.color, weight: 2.5, opacity: 0.9 }
        });
        geoJsonLayers[layerName] = layer;
        layer.addTo(map);
    }).catch((error) => {
        waterFeatureLineDataCache.delete(cacheKey);
        console.error(`Error loading water-feature lines for ${layerName}:`, error);
    });
}

function isTankFeature(properties, tankType) {
    const sourceProperties = properties || {};
    const featureCode = String(sourceProperties.HG_GFCODE || sourceProperties.HP_GFCODE || '').trim().toUpperCase();
    const featureName = String(sourceProperties.HG_Nm_Hydr || sourceProperties.HP_Nm_Hydr || '').trim().toLowerCase();
    const isReservoir = featureCode.includes('RSVR') || featureName.includes('reservoir');
    const isTank = featureCode.includes('TANK') || featureCode.includes('TNK') || featureName.includes('tank');

    return tankType === 'major' ? isReservoir : isTank && !isReservoir;
}

function loadTankFeatureType(url, layerName, tankType) {
    fetch(url)
        .then((response) => {
            if (!response.ok) throw new Error('Network response was not ok');
            return response.json();
        ...
        .then((data) => {
            const filteredData = {
                ...data,
                features: (data.features || []).filter((feature) => isTankFeature(feature.properties, tankType))
            };
            const layer = L.geoJSON(filteredData, {
                style: layerStyles[layerName] || { color: '#0077b6', fillColor: '#48cae4', weight: 1, fillOpacity: 0.6 }
            });
            geoJsonLayers[layerName] = layer;
            layer.addTo(map);
        })
...
function getWaterFeaturePointCode(properties) {
    const sourceProperties = properties || {};
    // The published GeoJSON uses HP_GFCODE. Keep HP_GFCODDE as a fallback
    // for files exported with that alternate field name.
    return String(sourceProperties.HP_GFCODE || sourceProperties.HP_GFCODDE || '').trim().toUpperCase();
}

function getWaterFeaturePointName(properties) {
    const name = properties && properties.HP_Nm_Hydr;
    if (name === null || name === undefined) return '';
    const label = String(name).trim();
    return label.toLowerCase() === 'null' ? '' : label;
}

function createWaterFeaturePointMarker(code, latlng) {
    const iconByCode = {
        ANCTP: '🧱', // Anicut
        CLVTP: '🌉'  // Culvert
    };
    const colorByCode = {
        ANCTP: '#d73027', // Anicut
        FLGAP: '#fc8d59', // Flood Gate
        SLCEP: '#fee08b', // Sluice
        SPLLP: '#91cf60', // Spill
        WTRFP: '#1a9850', // Waterfall
        CLVTP: '#4575b4', // Culvert
        SPRNP: '#74add1', // Spring
        TOWRP: '#984ea3', // Water Tower
        TBWLP: '#e7298a', // Tube Well
        WELLP: '#8c510a'  // Well
    };
    const color = colorByCode[code] || '#35b9ff';
...
function loadWaterFeaturePointType(url, layerName, code) {
    const cacheKey = String(url || '').trim();
    const dataPromise = waterFeaturePointDataCache.has(cacheKey) ? waterFeaturePointDataCache.get(cacheKey) : fetch(url).then((response) => {
        if (!response.ok) throw new Error('Network response was not ok');
        return response.json();
    });
    waterFeaturePointDataCache.set(cacheKey, dataPromise);

    dataPromise.then((data) => {
        if (!layerState[layerName] || geoJsonLayers[layerName]) return;
        const filteredData = {
            type: 'FeatureCollection',
            features: (Array.isArray(data && data.features) ? data.features : []).filter((feature) =>
                getWaterFeaturePointCode(feature && feature.properties) === code
            )
        };
        const geoJsonLayer = L.geoJSON(filteredData, {
            pointToLayer: (feature, latlng) => {
                const marker = createWaterFeaturePointMarker(code, latlng);
                const name = getWaterFeaturePointName(feature && feature.properties);
                if (name) {
                    marker.bindTooltip(name, {
                        permanent: true,
                        direction: 'top',
                        offset: [0, -5],
                        className: 'water-feature-point-label',
                        opacity: 0.9
                    });
                }
                return marker;
        ...
        waterFeaturePointDataCache.delete(cacheKey);
        console.error(`Error loading water-feature points for ${layerName}:`, error);
    });
}
...
// Keep tank layers together in their own Base Layers category. The source CSV
// can place these layers in different databases, so build this display group
// after reading the CSV rather than relying on its Database column.
function addReservoirTankCategory(databaseLayers) {
    ...
    Object.keys(databaseLayers).forEach((databaseName) => {
        ...
        if (remainingLayers.length) {
            databaseLayers[databaseName] = remainingLayers;
        } else {
            delete databaseLayers[databaseName];
        }
    });
            ...
            .then(csvData => {
                Papa.parse(csvData, {
                    ...
                    complete: function (results) {
                        ...
                        updateBasePanel(addReservoirTankCategory(databaseLayers));
                    }
                });
            })
            .catch(error => {
...
function updateBasePanel(databaseLayers) {
    ...
    Object.keys(databaseLayers).forEach(databaseName => {
        ...
        databaseLayers[databaseName].forEach((layerInfo, index) => {
            ...
            if (isWaterFeaturesPointLayer(Layer)) {
                const pointGroup = document.createElement('div');
                const pointTitle = document.createElement('strong');
                pointTitle.textContent = Layer;
                pointGroup.appendChild(pointTitle);

                waterFeaturePointTypes.forEach((pointType) => {
                    const pointLayerName = `${Layer} - ${pointType.name}`;
                    const pointDiv = document.createElement('div');
                    pointDiv.style.marginLeft = '16px';
                    const pointCheckbox = document.createElement('input');
                    pointCheckbox.type = 'checkbox';
                    pointCheckbox.id = `layer_${databaseName}_${index}_${pointType.code}`;
                    pointCheckbox.checked = layerState[pointLayerName] || false;
                    pointCheckbox.addEventListener('change', (event) => {
                        const isChecked = event.target.checked;
                        layerState[pointLayerName] = isChecked;
                        if (isChecked) {
                            loadWaterFeaturePointType(geojson, pointLayerName, pointType.code);
                        } else if (geoJsonLayers[pointLayerName]) {
                            map.removeLayer(geoJsonLayers[pointLayerName]);
                            delete geoJsonLayers[pointLayerName];
                        }
                    });
                    const pointLabel = document.createElement('label');
                    pointLabel.htmlFor = pointCheckbox.id;
                    pointLabel.textContent = pointType.name;
                    pointDiv.appendChild(pointCheckbox);
                    pointDiv.appendChild(pointLabel);
                    pointGroup.appendChild(pointDiv);
                });
                databaseDiv.appendChild(pointGroup);
                return;
            }

            if (isWaterFeaturesLineLayer(Layer)) {
                const lineGroup = document.createElement('div');
                const lineTitle = document.createElement('strong');
                lineTitle.textContent = Layer;
                lineGroup.appendChild(lineTitle);

                waterFeatureLineTypes.forEach((lineType) => {
                    const lineLayerName = `${Layer} - ${lineType.name}`;
                    const lineDiv = document.createElement('div');
                    lineDiv.style.marginLeft = '16px';
                    const lineCheckbox = document.createElement('input');
                    lineCheckbox.type = 'checkbox';
                    lineCheckbox.id = `layer_${databaseName}_${index}_${lineType.name.toLowerCase()}`;
                    lineCheckbox.checked = layerState[lineLayerName] || false;
                    lineCheckbox.addEventListener('change', (event) => {
                        const isChecked = event.target.checked;
                        layerState[lineLayerName] = isChecked;
                        if (isChecked) {
                            loadWaterFeatureLineType(geojson, lineLayerName, lineType);
                        } else if (geoJsonLayers[lineLayerName]) {
                            map.removeLayer(geoJsonLayers[lineLayerName]);
                            delete geoJsonLayers[lineLayerName];
                        }
                    });
                    const lineLabel = document.createElement('label');
                    lineLabel.htmlFor = lineCheckbox.id;
                    lineLabel.textContent = lineType.name;
                    lineDiv.appendChild(lineCheckbox);
                    lineDiv.appendChild(lineLabel);
                    lineGroup.appendChild(lineDiv);
                });
                databaseDiv.appendChild(lineGroup);
                return;
            }
